		...
		if ($keyword) {
				...
				if ($filter == "all" or !$filter) {
						...
						while (list($key, $val) = each($filtro)) {
							    ...
							    if (!is_array($ignorar)) { 
									if ($tmp == True) {$where_arr[] = "$key = '$keyword'";}
									else {$where_arr[] = "$key ILIKE '%$keyword%'";}
								}	
						}
...
function link_calendario($control_pos, $control_dat="") {
	...
	return "<script  src='/nacer/lib/popcalendar.js'></script> <img src=$html_root/imagenes/cal.gif border=0 align=middle style='cursor:hand;' alt='Haga click aqui para\nseleccionar la fecha'  onClick=\"javascript:popUpCalendar($control_pos, $control_dat, 'dd/mm/yyyy');\">";
}
