   ...
   /*$usuario = substr($usuario,0,9);*/
	...
	If ($estado_envio== 'n'){
		if($responsable =='MADRE'){
             ...
             nro_doc_tutor='',tipo_doc_tutor='', activo='1'
			...
			$query = "update uad.beneficiarios set 
             ...
             apellido_tutor='', nro_doc_tutor='',tipo_doc_tutor='', activo='1' 
			 where id_beneficiarios=".$id_planilla;
             ...
             apellido_padre='', nro_doc_padre='',tipo_doc_padre='', activo='1' 
             where id_beneficiarios=".$id_planilla;
  		 }
		...
		if($responsable =='MADRE'){
	             ...
	             tipo_doc_madre=upper('$tipo_doc_madre'),nombre_padre='',apellido_padre='', 
	             nro_doc_padre='',tipo_doc_padre='',nombre_tutor='', apellido_tutor='', 
	             nro_doc_tutor='',tipo_doc_tutor='', tipo_transaccion='M', estado_envio='n',activo='1'
	   		...
	   		$query = "update uad.beneficiarios set 
	             ...
	             tipo_doc_padre=upper('$tipo_doc_madre'),nombre_madre='', 
	             apellido_madre='', nro_doc_madre='',tipo_doc_madre='',nombre_tutor='', 
	             apellido_tutor='', nro_doc_tutor='',tipo_doc_tutor='', tipo_transaccion='M', estado_envio='n',activo='1'                        
	         ...
	         where id_beneficiarios=".$id_planilla;
	             ...
	             tipo_doc_tutor=upper('$tipo_doc_madre'),nombre_madre='', 
	             apellido_madre='', nro_doc_madre='',tipo_doc_madre='',nombre_padre='',
	             apellido_padre='', nro_doc_padre='',tipo_doc_padre='', tipo_transaccion='M', estado_envio='n',activo='1'                              
	  		 ...
	  		 if ($id_categoria=='1')  {
				 ...
				 telefono='$telefono',cod_pos='$cod_posn',observaciones=upper('$observaciones'),fecha_carga='$fecha_carga', usuario_carga=upper('$usuario'),
				 fecha_inscripcion='$fecha_inscripcion', provincia_nac=upper('$provincia_nacn'), localidad_nac=upper('$localidad_procn'), pais_nac=upper('$pais_nacn'), 
				 tipo_ficha='1', tipo_transaccion='M', estado_envio='n',activo='1'
...
<!-- <script type="text/javascript" src="/lib/jquery-1.5.1.js"> </script>  La Pampa -->
<script>
...
function esFechaValida(fecha){
    ...
    switch(mes){
		...
		hoy=new Date() 
		if (anio<1800 || anio>hoy.getYear() + 3){
            alert("Fecha introducida errónea");
            return false;
        }
...
function control_nuevos()
{
 ...
 if ((document.all.id_categoria.value=='3')||(document.all.id_categoria.value=='4')){
			...
			if ((document.all.nro_doc_madre.value != document.all.num_doc.value) && (document.all.clase_doc.value == 'A')){
				alert( "El niño con DNI ajeno debe tener el mismo DNI que el responsable "); 
	...
	//Control de fechas futuras en embarazadas
	if (document.all.id_categoria.value=='1'){
		if ((esFechaValida(document.all.fecha_diagnostico_embarazo)) == false) {
			alert("La fecha de diagnostico de embarazo no tiene el formato correcto");
			document.all.fecha_diagnostico_embarazo.focus();
			return false;
		}
		if ((esFechaValida(document.all.fecha_probable_parto)) == false) {
			alert("La fecha de probable de parto no tiene el formato correcto");
			document.all.fecha_probable_parto.focus();
			return false;
		}
		if ((esFechaFutura(document.all.fecha_diagnostico_embarazo)) == false) {
			alert("La fecha de diagnostico de embarazo no puede ser mayor que la fecha actual");
			document.all.fecha_diagnostico_embarazo.focus();
			return false;
		}
	}
	
	//Control de fechas futuras en puerperas
	if (document.all.id_categoria.value=='2'){
		if ((esFechaValida(document.all.fecha_efectiva_parto)) == false) {
			alert("La fecha efectiva de parto no tiene el formato correcto");
			document.all.fecha_efectiva_parto.focus();
			return false;
		}
		if ((esFechaFutura(document.all.fecha_efectiva_parto)) == false) {
			alert("La fecha efectiva de parto no puede ser mayor que la fecha actual");
			document.all.fecha_efectiva_parto.focus();
			return false;
		}
	}
	...
	//Control de fecha de nacimiento completa
	...
	//Control de que no hayan hombres embarazados
	if ((document.all.id_categoria.value=='1')||(document.all.id_categoria.value=='2')){
		if(document.all.sexo.value=='M'){
			alert("No pueden haber hombres embarazados");
			document.all.sexo.focus();
			return false;
		} 
	}
	
	//Control de que no hayan niñas embarazados
	if ((document.all.id_categoria.value=='1')||(document.all.id_categoria.value=='2')){
		if (calcular_edad(document.all.fecha_nac.value) < 10){
			alert("No pueden haber niños menores a 10 años embarazados");
			alert(calcular_edad(document.all.fecha_nac.value));
			document.all.fecha_nac.focus();
			return false;
		} 
	}
...
<input type="hidden" value="<?= date("Ymd")?>" name="fechahoy" id="fechahoy">
		...
		<tr>
         ...
         <tr>
			...
			<select name=responsable Style="width=200px" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> >
			  <option value=RESPONSABLE <?if ($responsable=='') echo "selected"?>>RESPONSABLE</option>
		...
		<tr>
         ...
         <tr>
		    ...
		    <td align="left" colspan="2">
		    	 <input type=text name=fecha_inscripcion id=fecha_inscripcion onblur="esFechaValida(this);" value='<?=$fecha_inscripcion;?>' size=15 onkeypress="return pulsar(event)" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?>>
...
<table class="bordes" align="center" width="100%">
		 ...
		 <tr>
		    <td align="center">
		      ...
		      <!-- <input type="submit" name="transferir" value="Transferir" title="Transferir" style="width=130px"> -->
 ...
 <tr><td><table width=100% align="center" class="bordes">
  <tr align="center">
   <td>
	 ...
	 <input type="button" name="verfecha" value="verfecha" title="verfecha" style="width=130px" onclick="esFechaFutura('01/01/2013')"> 
  </td>
  </tr>
