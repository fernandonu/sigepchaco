...
$orden = array(
		...
		"2" => "beneficiarios.apellido_benef",
...
// Armar nombre del Archivo A conseguir ultima parte, secuencia final.
...
if (strlen($seq) < 5) {
	$seq = str_repeat("0",5-strlen($seq)).$seq;
}
// Fin datos para armar nombre de archivo A
       
$sql_tmp="SELECT to_char(fecha_nacimiento_benef, 'DD-MM-YYYY') fecnacformat, beneficiarios.*, efe_conv.nombreefector FROM uad.beneficiarios
...
if (permisos_check("inicio","genera_archivo_permiso")) 
	$permiso="";
else
	$permiso="disabled";
...
<form name=form1 action="ins_listado.php" method=POST>
	...
	<div class="row-fluid">
		...
		<div class="span4">
		<input class="btn btn-primary pull-right" type='button' name="nuevo" value='Inscribir' onclick="document.location='ins_admin.php'">
...
if ($_POST['generarnino']){
    	...
    	if (!$result1->EOF) {
			...
			if ($result1->fields['tipo_transaccion']== 'M'){
				$contenido.="111111111111111111111111111111111111111111111111111111111111111111111";
				$contenido.=chr(9);
...
?>
	<div class="pull-right">Beneficiarios Encontrados: <?=$total_muletos?> <?=$link_pagina?></div>
	...
	<table class="table table-condensed table-bordered table-hover">
		...
		<tr>
			...
			<th><a href='<?=encode_link("ins_listado.php",array("sort"=>"1","up"=>$up))?>'>Documento</a></th>      	    
			<th><a href='<?=encode_link("ins_listado.php",array("sort"=>"2","up"=>$up))?>'>Apellido</a></th>      	    
			<th>Nombre</th>
			<th>Nacimiento</a></th>
			<th><a href='<?=encode_link("ins_listado.php",array("sort"=>"3","up"=>$up))?>'>Efector</a></th>
	...
	?>
		...
		<tr class="manito">     
			...
			<td onclick="<?=$onclick_elegir?>"><?=$result->fields['fecnacformat']?></td>
			...
			<?
				if ($cmd!="d") {
					...
					echo "<a target='_blank' href='".$link."' title='Imprime Certificado'><i class='icon-print'></i> Imprimir</a>";
				}
			?> 
			</td>
		</tr>
