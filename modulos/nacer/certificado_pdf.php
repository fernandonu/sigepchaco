...
include_once("certif_act_clasepdf.php"); 
...
$query="SELECT   
  ...
  nacer.smiafiliados.maapellido,
  nacer.smiafiliados.manombre
FROM
  nacer.smiafiliados
...
$query2="SELECT MAX(PERIODO) periodo FROM nacer.smiprocesoafiliados";
$f_per=$db->Execute($query2) or die($db->ErrorMsg());
...
$pdf->afiactivo($f_res->fields['activo']);
$pdf->manom($f_res->fields['maapellido']);
$pdf->manrodoc($f_res->fields['manrodocumento']);
$pdf->mes_ano($f_per->fields['periodo']);
