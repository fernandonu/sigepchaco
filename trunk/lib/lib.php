...
//				$up = $parametros["up"];
		...
		if ($keyword) {
				...
				if ($filter == "all" or !$filter) {
						...
						while (list($key, $val) = each($filtro)) {
							    if (is_array($ignorar) && !in_array($key,$ignorar))
							     $where_arr[] = "CAST ($key as TEXT) ILIKE '%$keyword%'";
							    if (!is_array($ignorar)) $where_arr[] = "CAST ($key as TEXT) ILIKE '%$keyword%'";
				...
				else {if (!is_array($ignorar)) $where .= "CAST ($filter as TEXT) ILIKE '%$keyword%'";
...
// Busqueda PUCO Horacio Rio Negro ---
function form_busqueda_puco($sql,$orden,$filtro,$link_pagina,$where_extra="",$contar=0,$sumas="",$ignorar="",$seleccion="") {

/*es igual que el otro pero no usa %% en el like, hay que ponerlo a mano
saque ILIKE y puse LIKE*/
...
if (!is_array($ignorar)) $where_arr[] = "$key LIKE '$keyword'";
...
else {if (!is_array($ignorar)) $where .= "CAST ($filter as TEXT) LIKE '$keyword'";
elseif (is_array($ignorar) && !in_array($filter,$ignorar))
...
$pos = stripos($sql, 'puco.puco'); //select sobre puco?
if ($pos !== false) {
...
if ($saltea_contar) {
$total =0;
$result = sql('select * from nacer.parametros where 1=2') or die; //tomo cualquier tabla para que de result vacio
}
else {
$tipo_res = db_tipo_res("a");
// $result = sql($sql,"CONTAR") or reportar_error($sql,__FILE__,__LINE__);
...
function link_calendario($control_pos, $control_dat="") {
	...
	return "<script  src='$html_root/lib/popcalendar.js'></script> <img src=$html_root/imagenes/cal.gif border=0 align=middle style='cursor:pointer;' alt='Haga click aqui para\nseleccionar la fecha'  onClick=\"javascript:popUpCalendar($control_pos, $control_dat, 'dd/mm/yyyy');\">";
}
...
function generar_barra_nav($campos_barra) {
	...
	$barra = "<div class='newstyle-submenu'>
	...
	foreach ($campos_barra as $clave => $valor) {
		...
		if ($cmd == $valor["cmd"]) {
			...
			$barra .= '<li class="active"><a href="'.encode_link($_SERVER["PHP_SELF"],is_array($valor["extra"])?array_merge($valor["extra"],array("cmd" => $valor["cmd"])):array("cmd" => $valor["cmd"])).'">'.$valor["descripcion"].'</a></li>';
		} else {
			//$menuid="ma"." background='$html_root/imagenes/btn_azul.gif'";
			//$barra .= "<td id=$menuid class='bordesderinferior' width='$width%' style='cursor:pointer' onmouseover=\"this.style.color='#F7BE81'\" onmouseout=\"this.style.color='#2D3053'\" >";
			//$barra .= '<a href="'.encode_link($_SERVER["PHP_SELF"],is_array($valor["extra"])?array_merge($valor["extra"],array("cmd" => $valor["cmd"])):array("cmd" => $valor["cmd"])).'">';
			//$barra.=$valor["descripcion"]."</a></td>";
			$barra .= '<li><a href="'.encode_link($_SERVER["PHP_SELF"],is_array($valor["extra"])?array_merge($valor["extra"],array("cmd" => $valor["cmd"])):array("cmd" => $valor["cmd"])).'">'.$valor["descripcion"].'</a></li>';
		}
	}
