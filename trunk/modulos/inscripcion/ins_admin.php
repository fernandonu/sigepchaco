		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data) {
					...
					$('#beneficiarios_list tr').click(function() {
						$(this).find('td label input:radio').prop('checked', true);
					});

					$("#modal_message").html("<strong>Atencion!</strong> El documento ingresado se corresponde con los siguientes beneficiarios.");
					$("#modal_message").removeClass().addClass("alert alert-info");
					$("#modal_default_buttons").hide();
					$("#modal_success_buttons").show();
			...
			error: function (xhRequest, ErrorText, thrownError) {
				$("#modal_message").text("Error: " + ErrorText);
				$("#modal_message").removeClass().addClass("alert alert-error")
				$("#beneficiarios_result").modal('show');
			}
		});
...
<form name='form1' action='ins_admin.php' accept-charset="utf-8" method='POST'>
	...
	<p class="legend legend form-title">
	...
	<? 
		if ($id_categoria!='6'){ 
	?>
		<div class="alert color-fieldset">
			<p class="legend form-title">
...
<div class="modal hide" id="beneficiarios_result" tabindex="-1" aria-hidden="true">
	...
	<div class="modal-body">
		<div id="modal_message" class="alert alert-warning">
			Se encontraron beneficiarios con el DNI ingresado. 
			Seleccione uno para modificarlo. Haga click en cancelar para inscribir uno nuevo.
		</div>

		<table id="beneficiarios_list" class="table table-condensed table-hover">
		</table>
	</div>
	<div class="modal-footer">
		<div id="modal_success_buttons">
